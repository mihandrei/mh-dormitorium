...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		{
			char* materialfname = strtok(NULL, WHITESPACE);
			int dirlen = 0;
			if(materialfname[0] == '.'){//copy path for relative dirs
				size_t pathlen = strlen(filename);
				char* fnm = strrchr(filename,'/');
				if(fnm!=NULL){
					size_t fnamelen = strlen(fnm)-1;
					dirlen = pathlen - fnamelen;
					dirlen = dirlen>OBJ_FILENAME_LENGTH?OBJ_FILENAME_LENGTH:dirlen;
					strncpy(growable_data->material_filename, filename, dirlen);
				}
			}

			strncpy(growable_data->material_filename + dirlen,
					materialfname, OBJ_FILENAME_LENGTH - dirlen);
